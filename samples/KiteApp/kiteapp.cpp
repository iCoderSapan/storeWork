...
//std::cout<<builder.to_string()<<"\n"<<body_data<<std::endl;
pplx::task<web::http::http_response> resp =  client.request(methods::POST, utility::conversions::to_utf8string(builder.to_string()), builder.to_string(),
                    utility::conversions::to_utf8string("application/x-www-form-urlencoded"));
try { 
        //-- All Taskss will get triggered here 
        resp.wait();
        std::cout<<"Response received:\n";
        std::cerr<<resp.get().extract_json().get();
} 
catch (const std::exception &e) { 
        printf("Error exception:%sn", e.what()); 
}
